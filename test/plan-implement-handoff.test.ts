...
test("exact PLAN_AUTHORIZE와 ready scope를 deterministic ImplementContract로 변환한다", () => {
  ...
  const contract = createPlanImplementContract(approved, readyPlan);
...
test("ready=false, blocking question, unsafe scope와 비허용 검증 명령은 handoff하지 않는다", () => {
  const approved = authorization();
  assert.throws(
    () => createPlanImplementContract(approved, { ...readyPlan, implementationScope: { ...readyPlan.implementationScope, ready: false } }),
    /not ready/,
  );
  assert.throws(
    () => createPlanImplementContract(approved, { ...readyPlan, questions: ["결정 필요"] }),
    /blocking questions/,
  );
  assert.throws(
    () => createPlanImplementContract(approved, {
      ...readyPlan,
